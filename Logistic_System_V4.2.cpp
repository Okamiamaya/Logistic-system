...
// (a rolling/polynomial hash) so the dispatch table can look up the intended
// command in O(1) average time instead of doing string comparisons.
...
// ---------------------------------------------------------------------------
// Command handlers — one function per user-facing action. These are the
// same handlers as the enum version; only the dispatch mechanism differs.
// ---------------------------------------------------------------------------
...
// buildDispatchTable()
// ---------------------------------------------------------------------------
// Purpose: Populates a hash -> handler lookup table. Both the full-word hash
// and the numeric shortcut for a command point to the same std::function, so
// either form of input resolves to the same handler with a single map
// lookup. This table is the command-determination mechanism for this
// version of the program and is left structurally unchanged; adding a new
// command means adding one more pair of entries here.
void buildDispatchTable(unordered_map<HashType, function<void()>>& dispatchTable){
    dispatchTable[285516734]=cmdAddLab;                       dispatchTable[1]=cmdAddLab;
    dispatchTable[844736042]=cmdRemoveLab;                     dispatchTable[2]=cmdRemoveLab;
    dispatchTable[961457947]=cmdSearchLab;                     dispatchTable[3]=cmdSearchLab;
    dispatchTable[403052850]=cmdSearchLabClassification;       dispatchTable[4]=cmdSearchLabClassification;
    dispatchTable[851355098]=cmdSearchAllLabClassification;    dispatchTable[5]=cmdSearchAllLabClassification;
    dispatchTable[276693854]=cmdSearchSupplyClassification;    dispatchTable[6]=cmdSearchSupplyClassification;
    dispatchTable[341807955]=cmdAddSupply;                     dispatchTable[7]=cmdAddSupply;
    dispatchTable[781609042]=cmdRemoveSupply;                  dispatchTable[8]=cmdRemoveSupply;
    dispatchTable[474046146]=cmdSearchSupply;                  dispatchTable[9]=cmdSearchSupply;
    dispatchTable[238344928]=cmdSearchUnclassifiedData;        dispatchTable[42]=cmdSearchUnclassifiedData;
    dispatchTable[794087791]=cmdDeleteUnclassifiedData;        dispatchTable[43]=cmdDeleteUnclassifiedData;
    dispatchTable[149426950]=cmdClassifyData;                  dispatchTable[44]=cmdClassifyData;
    dispatchTable[1816364]=cmdHelp;                             dispatchTable[45]=cmdHelp;
    dispatchTable[937733000]=cmdEndSystem;                      dispatchTable[46]=cmdEndSystem;
}

// ---------------------------------------------------------------------------
// main() — looks up the hashed command directly in the dispatch table and
// calls the matching handler in O(1) average time. No switch/if-else chain
// is needed at all; an unrecognized hash simply isn't in the table.
// ---------------------------------------------------------------------------
int main(){
    ...
    unordered_map<HashType, function<void()>> dispatchTable;
    ...
    while(systemRunning){
        ...
        auto handlerIterator=dispatchTable.find(hashValue);
        if(handlerIterator!=dispatchTable.end()) handlerIterator->second();
        else{
